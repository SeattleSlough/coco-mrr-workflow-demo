...
| Branch | Contents |
| :---- | :---- |
| `main` | Schema setup \+ the Pillar 1 CoCo baseline, merged |
| `act1-manual` | Pillar 1 (Build), schema-blind arm |
| `act1-coco` | Pillar 1 (Build), CoCo arm |
| `act2-manual` | Pillar 2 (Evolve), schema-blind arm |
| `act2-coco` | Pillar 2 (Evolve), CoCo arm |
...
- [dbt-core](https://docs.getdbt.com/) with the Snowflake adapter (`pip install dbt-core dbt-snowflake`)
- A `~/.dbt/profiles.yml` pointing at your account (see `sql/schema.sql` for the database/schema this project expects)
- For the CoCo branches: [Cortex Code](https://www.snowflake.com/en/blog/cortex-code-snowsight/), configured with a valid Snowflake connection

## Running it

1. Create the schema and seed data (run once, in Snowsight or via your preferred SQL client):

```shell
sql/schema.sql
```

2. Clone and check out whichever branch you want to inspect or run:
...
3. Confirm your connection and build the models:
